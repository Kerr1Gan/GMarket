...
export default {
  mounted() {
    let ctx = this;
    Vue.http.get("/api/market/api/app").then(
      response => {
        // get body data
    ...
    Vue.http.get("/api/market/api/appProducts?productId=2").then(
      response => {
        // get body data
        console.log(response.body);
        let array = new Array();
  ...
  methods: {
    ...
    updateProduct(product) {
      ...
      Vue.http.post("/api/market/api/appProducts", productDto).then(
        response => {
          // get body data
          console.log(response.body);
